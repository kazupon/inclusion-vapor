...
export function genCssVarsFromList(
  ...
  return `{\n  ${vars
    .map(key => `"${isSSR ? `--` : ``}${genVarName(id, key, isProd, isSSR)}": (${key})`)
    .join(',\n  ')}\n}`
}
...
function genVarName(id: string, raw: string, isProd: boolean, isSSR = false): string {
  ...
  // into an HTML string
  return isProd ? hash(id + raw) : `${id}-${getEscapedCssVarName(raw, isSSR)}`
}
...
//     const content = style.content.replaceAll(/\/\*([\s\S]*?)\*\/|\/\/.*/g, '')
